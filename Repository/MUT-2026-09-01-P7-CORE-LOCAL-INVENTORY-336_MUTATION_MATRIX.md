...
- No current historical discoverability regression survived from aborted P136, so P336 adds a new direct local regression in the same functional change set.

## Authorized functional change set
| Change | Target | Action | Applied | Verified |
|---|---|---|---:|---:|
| 336-01 | `Core/Core.md` | UPDATE local inventory to include CORE-012 and current reconciliation boundary | Y | N |
| 336-02 | `Core/_FOLDER_STATUS.md` | UPDATE exact local inventory/re-audit state while preserving Integrity Hold and certification hold | Y | N |
| 336-03 | `Quality/Integration/test_core_local_inventory_reconciliation.py` | CREATE direct exact-inventory regression | Y | N |
| 336-04 | `Repository/P7_CORE_LOCAL_INVENTORY_RECONCILIATION_336_2026-09-01.md` | CREATE bounded P7 progress record | Y | N |
| 336-05 | `Repository/REP-016_PRIORITY7_PROGRESS_ADDENDUM_2026-09-01_P336.md` | CREATE current P7 operational progress addendum | Y | N |
| 336-06 | `Repository/REP-011_PRIORITY7_PROGRESS_ADDENDUM_2026-09-01_P336.md` | CREATE traceability addendum | Y | N |
| 336-07 | this Matrix | UPDATE in same functional change set | Y | N |
...
## Applied design
- `Core/Core.md` now defines its member set as exact top-level Core files excluding only the self-index file and includes CORE-012.
...
`PRIORITY 7 = IN_PROGRESS / LOCAL CORE INVENTORY RECONCILED-CANDIDATE / CONTROL-PLANE INVENTORY + CROSS-LAYER VALIDATION OPEN`.

P336 may close only the Core-local inventory/index/status subgate after exact-head verification. It must not close Priority 7.
