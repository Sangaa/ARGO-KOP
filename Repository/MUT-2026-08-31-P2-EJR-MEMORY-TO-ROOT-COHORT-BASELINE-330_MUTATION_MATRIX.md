...
|---|---|---|---|---|---|
| 330-01 | `Quality/Integration/ejr_memory_to_root_provenance_census.py` | UPDATE | `EXPECTED_GROUP_COUNT = 5` → `4` only | N | N |
...
## KEEP REQUIREMENT
This is a rebaseline-only lease. No identity disposition, rename, delete, allocation, semantic consumer rewrite, Runtime mutation, REP-016 mutation, priority promotion, or census logic change is authorized. Change only the single expected cohort constant 5→4 and include this Matrix in the same functional change set.
