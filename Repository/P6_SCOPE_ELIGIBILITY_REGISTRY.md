...
`OUT_OF_SCOPE` is bounded: it means P6 direct impact correlation does not apply to that changed-path class. It does not mean the artifact has no evidentiary, historical, learning, governance, or architectural value.
...
3. Existing Mutation Matrix preflight behavior treats `EJR/**` as documentation/session evidence rather than a protected implementation mutation path.
