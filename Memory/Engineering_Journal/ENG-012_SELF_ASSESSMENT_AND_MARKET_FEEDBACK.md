...
This learning record was originally created as `ENG-011`, but that identity already belongs to `Engine/ENG-011_MARITIME_GAME_ENGINE.md`. The duplicate was detected during repository review and corrected to `ENG-012` before the learning record was treated as canonical.

This correction is itself audit evidence: identifier uniqueness must be checked across the repository before creating new artifacts, not inferred from local folder numbering.
