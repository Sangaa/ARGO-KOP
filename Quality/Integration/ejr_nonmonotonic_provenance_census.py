"""Evidence-only provenance census for non-monotonic EJR ambiguity groups.

Lease 199 proved four H1-only groups (EJR-195..198) whose exact-path namespace
sequence is ROOT_EJR -> MEMORY_EJR -> ROOT_EJR.  This companion surfaces
current content and reference evidence only.  It deliberately emits no owner,
canonical, migration, rename, or suppression decision.
"""
...
TARGET_IDS = ("EJR-195", "EJR-196", "EJR-197", "EJR-198")
SELF_GENERATED_PATHS = {
    ...
    "Repository/MUT-2026-08-30-P2-EJR-NONMONOTONIC-PROVENANCE-CENSUS-200.md",
    "Repository/MUT-2026-08-30-P2-EJR-NONMONOTONIC-PROVENANCE-CENSUS-200_MUTATION_MATRIX.md",
    "Repository/ROOM071_RECONSTRUCTION_SUPPLEMENT_200_2026-08-30.md",
}
...
def _reference_paths(
    ...
    sibling_path_refs: dict[str, list[str]] = {path: [] for path in sorted(group_paths)}
    ...
    for relative in tracked:
        ...
        for member_path in sibling_path_refs:
            if member_path in text:
                sibling_path_refs[member_path].append(relative_str)

    return sorted(set(external_id_refs)), {
        path: sorted(set(refs)) for path, refs in sibling_path_refs.items()
    }
...
def classify_from_report(root: Path, report: dict[str, Any]) -> dict[str, Any]:
    ...
    for document_id in TARGET_IDS:
        ...
        if len(member_paths) != 3 or sources != ["FIRST_H1_FALLBACK"]:
            incomplete.append(document_id)
            groups[document_id] = {
        ...
        external_id_refs, sibling_path_refs = _reference_paths(
            root, tracked, set(member_paths), document_id
        )
        digests = [item["content_sha256"] for item in observations]
        groups[document_id] = {
            "membership_state": "EXPECTED_THREE_H1_MEMBERS",
            "member_count": 3,
            "identity_sources": sources,
            ...
            "external_exact_sibling_path_references": sibling_path_refs,
            "members": observations,
        }
    ...
    return {
        ...
        "history_scope": "all locally reachable refs",
        "evidence_boundary": (
            "content digests/titles/current exact references only; no ownership, "
            "canonical, migration, rename, or suppression authority"
        ),
        "target_ids": list(TARGET_IDS),
