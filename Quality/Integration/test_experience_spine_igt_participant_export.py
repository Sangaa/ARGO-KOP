...
def _experience_packet() -> dict:
    return {
        "status": "READY", "task_id": "XSP-IGT-PACKET",
        "execution_identity": "HERMUZ:source-packet", "execution_context": {"consumer_route": "HERMUZ"},
        "experience_items": [{
            "knowledge_id": "K-RELEASE",
            "pattern": "Inspect current release gates before acting on historical launch guidance.",
            "knowledge_scope": "project:test", "lifecycle_state": "PROMOTED", "validation_state": "VALIDATED",
            "evidence": ["E-RELEASE-1"], "evidence_state": "PROVEN", "authority_state": "ADVISORY",
            "source_identity": "HERMUZ:prior-run", "source_type": "HERMUZ-ENGINEERING",
            "evidence_group": "EG-RELEASE", "consumer_routes": ["HERMUZ"],
            "applicability_boundaries": ["release-operations"],
            "counterindications": ["current gate supersedes old launch condition"], "contradicts": [],
            "match_reasons": {"problem_types": ["release-gate-conflict"]}, "score": 2,
        }],
        "conflicts": [],
        "correlated_evidence_groups": [{
            "evidence_group": "EG-RELEASE", "knowledge_ids": ["K-RELEASE"],
...
def _build(case_id="XSP-IGT-01", condition="B0") -> dict:
    kwargs = {
        "experiment_id": "IGT-REAL-RUN-001", "case_id": case_id,
        "condition": condition, "baseline_sha": BASELINE,
    }
...
def test_all_six_participant_exports_are_locally_verified(case_id, condition):
    ...
    assert verify_participant_export(package) == {
        "state": "VERIFIED_PARTICIPANT_EXPORT", "reasons": [],
...
def test_exact_full_baseline_sha_is_required():
    for bad in ("main", "45ed9275", "Z" * 40, ""):
        with pytest.raises(ValueError, match="FULL_BASELINE_SHA_REQUIRED"):
            build_participant_export(experiment_id="EXP", case_id="XSP-IGT-01", condition="B0", baseline_sha=bad)
...
def test_condition_information_boundaries_are_preserved_in_export():
    ...
    assert "experience_packet" not in b0 and "provenance_envelope" not in b0
    assert "experience_packet" in l1 and "provenance_envelope" not in l1
    assert l2["experience_packet"] == l1["experience_packet"] and "provenance_envelope" in l2
...
def test_post_export_payload_mutation_breaks_digest_and_export_identity():
    mutated = deepcopy(_build("XSP-IGT-01", "B0"))
...
def test_export_cannot_pretend_execution_already_happened():
    ...
    package["execution_evidence"] = {
        "state": "MODEL_RUN", "participant_evidence_ref": "evidence://invented", "provider_receipt": "receipt://invented",
    }
    result = verify_participant_export(package)
    assert result["state"] == "INVALID"
...
def test_export_generation_never_populates_participant_rows_or_authenticity():
    ...
    result = verify_participant_export(package)
    serialized = repr(package)
    for forbidden in ("execution_context_id", "independence_attestation_ref", "provider_request_id", "external_authenticity"):
