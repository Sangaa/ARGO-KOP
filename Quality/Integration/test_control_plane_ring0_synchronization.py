...
def test_ring0_control_plane_artifacts_are_present_and_cross_referenced():
    ...
    for doc_id, content in contents.items():
        assert f"Document ID: {doc_id}" in content or f"Document ID\n{doc_id}" in content
    ...
    # REP-016 coordinates the control-plane set and keeps Ring 0 explicitly open.
    queue = contents["REP-016"]
    assert "RING 0 — CONTROL PLANE" in queue
    for doc_id in ("REP-011", "REP-012", "REP-013", "REP-014", "REP-015", "REP-020"):
        assert doc_id in queue
